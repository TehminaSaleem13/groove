...
RSpec.describe ExportsettingsController, type: :controller do
  ...
  describe 'Exportsettings' do
    ...
    it 'Export Order Include Orders with Box' do
      ...
      ProductBarcode.create(product_id: product.id, barcode: 'PRODUCT90', order: 0, lot_number: nil,
                            packing_count: '1', is_multipack_barcode: true)
      OrderItemKitProduct.create(order_item_id: order_item.id, product_kit_skus_id: 1,
                                 scanned_status: 'partially_scanned', scanned_qty: 1)
      ExportSetting.last.update(order_export_type: 'include_all')
      @user.role.update(view_packing_ex: true)
