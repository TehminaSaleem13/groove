...
RSpec.describe StoresController, type: :controller do
  ...
  describe 'CSV Import' do
    ...
    it 'Import CSV Products' do
      ...
      post :create_update_store,
           params: { store_type: 'CSV', import_user_assignments: false, status: store.status, name: store.name,
                     inventory_warehouse_id: store.inventory_warehouse_id, id: store.id, productfile: fixture_file_upload('csv_product_import.csv') }
      ...
      request.accept = 'application/json'
      post :csv_do_import,
           params: { id: store.id, rows: '2', sep: ',', other_sep: '0', delimiter: '"', fix_width: '0', fixed_width: '4',
                     contains_unique_order_items: false, generate_barcode_from_sku: false, use_sku_as_product_name: false, import_action: doc[:map][:import_action], map: doc[:map][:map], controller: 'stores', action: 'csv_do_import', store_id: store.id, name: doc[:name], type: 'product', flag: 'file_upload', encoding_format: 'UTF-8' }
    ...
    it 'Import Kit Products' do
      request.accept = 'application/json'
      post :create_update_store,
           params: { store_type: 'CSV', status: store.status, name: store.name,
                     inventory_warehouse_id: store.inventory_warehouse_id, id: store.id, kitfile: fixture_file_upload('csv_kit_import.csv') }
      expect(response.status).to eq(200)
      ...
      post :csv_do_import,
           params: { id: store.id, rows: '2', sep: ',', other_sep: '0', delimiter: '"', fix_width: '0', fixed_width: '4',
                     contains_unique_order_items: false, generate_barcode_from_sku: false, use_sku_as_product_name: false, import_action: doc[:map][:import_action], map: doc[:map][:map], controller: 'stores', action: 'csv_do_import', store_id: store.id, name: doc[:name], type: 'kit', flag: 'file_upload' }
  ...
  describe 'CSV Test and Imports' do
    ...
    it 'Remove Existing SKU and add new' do
      ...
      post :create_update_store,
           params: { store_type: 'CSV', status: store.status, name: store.name,
                     inventory_warehouse_id: store.inventory_warehouse_id, id: store.id, productfile: fixture_file_upload('MT_Products_remove_sku.csv') }
      expect(response.status).to eq(200)
    ...
    #   request.accept = 'application/json'
    #   post :create_update_store, params: { store_type: 'CSV', status: store.status, name: store.name, inventory_warehouse_id: store.inventory_warehouse_id, id: store.id, productfile: fixture_file_upload('csv_product_import.csv') }
    #   expect(response.status).to eq(200)
    ...
    it 'Do not remove SKU if only one skus is present' do
      ...
      post :csv_do_import,
           params: { id: store.id, rows: '2', sep: ',', other_sep: '0', delimiter: '"', fix_width: '0', fixed_width: '4',
                     contains_unique_order_items: false, generate_barcode_from_sku: false, use_sku_as_product_name: false, import_action: doc[:map][:import_action], map: doc[:map][:map], controller: 'stores', action: 'csv_do_import', store_id: store.id, name: doc[:name], type: 'product', flag: 'file_upload' }
    ...
    it 'Import via FTP' do
      ...
      FtpCredential.create(store_id: store.id, host: 'ecomdash-ftp.cloudapp.net/Inventory', username: 'DeJaViewedLLC',
                           password: 'DeJaViewedLLC123!', connection_method: 'ftp')
      request.accept = 'application/json'
    ...
    it 'Import via FTP without host' do
      store = Store.where(store_type: 'CSV', status: true).first
      FtpCredential.create(store_id: store.id, host: 'ecomdash-ftp.cloudapp.net', username: nil,
                           password: 'DeJaViewedLLC123!', connection_method: 'ftp')
      request.accept = 'application/json'
    ...
    it 'Import via SFTP' do
      store = Store.where(store_type: 'CSV', status: true).first
      FtpCredential.create(store_id: store.id, host: '34.202.114.73/groovepacker', username: 'groovepacker',
                           password: 'grOOvePacKeRRG!!', connection_method: 'sftp')
      request.accept = 'application/json'
    ...
    it 'Import Via SFTP without Username' do
      store = Store.where(store_type: 'CSV', status: true).first
      FtpCredential.create(store_id: store.id, host: '34.202.114.73/groovepacker', username: nil,
                           password: 'grOOvePacKeRRG!!', connection_method: 'sftp')
      request.accept = 'application/json'
  ...
  describe 'ShippingEasy Imports' do
    ...
    it 'show error if required field not mapped' do
      ...
      post :create_update_store,
           params: { store_type: 'CSV', status: store.status, name: store.name,
                     inventory_warehouse_id: store.inventory_warehouse_id, id: store.id, orderfile: fixture_file_upload('Order_import_fail.csv') }
      expect(response.status).to eq(200)
      ...
      post :csv_check_data,
           params: { id: store.id, map: doc[:map], controller: 'stores', action: 'csv_check_data', store_id: store.id, rows: 2,
                     sep: ',', other_sep: 0, delimiter: '"', fix_width: 0, fixed_width: 4, import_action: 'update_order', contains_unique_order_items: false, generate_barcode_from_sku: false, use_sku_as_product_name: false, order_date_time_format: 'Default', day_month_sequence: 'MM/DD', encoding_format: 'ASCII + UTF-8', type: 'order', name: 'Order_check_file_map_1', flag: 'file_upload', order_placed_at: '2020-05-25T11:37:36.352Z' }
      expect(response.status).to eq(200)
    ...
    it 'no error if file check passed' do
      ...
      post :create_update_store,
           params: { store_type: 'CSV', status: store.status, name: store.name,
                     inventory_warehouse_id: store.inventory_warehouse_id, id: store.id, orderfile: fixture_file_upload('Order_import_pass.csv') }
      expect(response.status).to eq(200)
  ...
  describe 'Shipstation' do
    ...
    it 'Shipstation Bin Location Pull' do
      create(:product, :with_sku_barcode, status: 'active', store_id: @ship_store.id, name: 'Test1',
  ...
  describe 'Veeqo Store' do
    ...
    it 'Update Veeqo Store' do
      post :create_update_store,
           params: { id: @veeqo_store.id, store_type: @veeqo_store.store_type, awaiting_fulfillment_status: false }
      expect(response.status).to eq(200)
    end
  end
  ...
  describe 'Shopify Imports' do
    ...
    before do
      ...
      shopify_store = Store.create(name: 'Shopify', status: true, store_type: 'Shopify',
                                   inventory_warehouse: InventoryWarehouse.last, on_demand_import: true)
      shopify_store_credentials = ShopifyCredential.create(store_id: shopify_store.id,
                                                           access_token: 'accesstokenaccesstoken', shop_name: 'test_shop')
    end
    ...
    it 'Update Shopify Store' do
      ...
      post :create_update_store,
           params: { id: store.id, shop_name: 'Shopify', store_type: store.store_type, add_gp_scanned_tag: true }
      expect(response.status).to eq(200)
    end
  ...
  describe 'Shopline Imports' do
    ...
    before do
      ...
      shopline_store = Store.create(name: 'Shopline', status: true, store_type: 'Shopline',
                                    inventory_warehouse: InventoryWarehouse.last, on_demand_import: true)
      shopline_store_credentials = ShoplineCredential.create(store_id: shopline_store.id,
                                                             access_token: 'accesstokenaccesstoken', shop_name: 'test_shopline')
    end
    ...
    it 'Update Shopline Store' do
      ...
      post :create_update_store,
           params: { id: store.id, shop_name: 'Shopline', store_type: store.store_type, add_gp_scanned_tag: false }
      expect(response.status).to eq(200)
    end
  ...
  describe 'Shippo Imports' do
    ...
    before do
      ...
      shippo_store_credentials = ShippoCredential.create(store_id: shippo_store.id,
                                                         api_key: 'shippo_test_6cf0a208229f428d9e913de45f83f849eb28d7d3', api_version: '2018-02-08')
    end
    ...
    it 'Shippo Store Settings' do
      ...
      shippo_store = Store.where(store_type: 'Shippo').last
      post :create_update_store,
           params: { id: shippo_store.id, store_type: shippo_store.store_type, status: true,
                     api_key: 'shippo_test_6cf0a208229f428d9e913de45f83f849eb28d7d3', api_version: '2018-02-08', generate_barcode_option: 'do_not_generate' }
      expect(response.status).to eq(200)
    end
    ...
    it 'Shippo Store Settings for Failed Status' do
      request.accept = 'application/json'
      post :create_update_store,
           params: { store_type: 'Shippo', status: true, api_key: 'shippo_test_6cf0a208229f428d9e913de45f83f849eb28d7d3',
                     api_version: '2018-02-08', generate_barcode_option: 'do_not_generate' }
      expect(response.status).to eq(200)
    end
  end
    ...
    before do
      ...
      @veeqo_store = create(:store, :veeqo, inventory_warehouse: inv_wh) do |store|
        store.veeqo_credential.update(store_id: store.id, shall_import_customer_notes: true,
                                      allow_duplicate_order: true, shall_import_internal_notes: true, gen_barcode_from_sku: true, import_shipped_having_tracking: true)
      end
    end
  ...
  describe 'Show Store' do
    ...
    before do
      ...
      shippo_store = create(:store, name: 'Shippo', status: true, store_type: 'Shippo', inventory_warehouse: inv_wh,
                                    on_demand_import: true)
      create(:shippo_credential, store_id: shippo_store.id,
                                 api_key: 'shippo_test_6cf0a208229f428d9e913de45f83f849eb28d7d3', api_version: '2018-02-08')
      veeqo_store = create(:store, name: 'Veeqo test', status: true, store_type: 'Veeqo', inventory_warehouse: inv_wh)
      create(:veeqo_credential, store_id: veeqo_store.id)
    end
  ...
  describe 'Ebay' do
    ...
    before do
      ...
      ebay_store = Store.create(name: 'Ebay', status: true, store_type: 'Ebay',
                                inventory_warehouse: InventoryWarehouse.last, on_demand_import: true)
  ...
  describe 'Amazon' do
    ...
    before do
      ...
      amazon_store = Store.create(name: 'Amazon', status: true, store_type: 'Amazon',
                                  inventory_warehouse: InventoryWarehouse.last, on_demand_import: true)
