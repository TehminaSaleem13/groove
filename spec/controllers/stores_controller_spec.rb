...
RSpec.describe StoresController, type: :controller do
  ...
  describe "CSV Import" do
    ...
    it 'Import CSV Products' do
      ...
      post :create_update_store, { store_type: 'CSV', status: @store.status, name: @store.name, inventory_warehouse_id: @store.inventory_warehouse_id, id: @store.id, productfile: fixture_file_upload(Rails.root.join('/files/csv_product_import.csv')) }
    ...
    it 'Import Kit Products' do
      request.accept = 'application/json'
      post :create_update_store, { store_type: 'CSV', status: @store.status, name: @store.name, inventory_warehouse_id: @store.inventory_warehouse_id, id: @store.id, kitfile: fixture_file_upload(Rails.root.join('/files/csv_kit_import.csv')) }
      expect(response.status).to eq(200)
      ...
      request.accept = 'application/json'
      post :csv_do_import, { id: @store.id, rows: '2', sep: ',', other_sep: '0', delimiter: '"', fix_width: '0', fixed_width: '4', contains_unique_order_items: false, generate_barcode_from_sku: false, use_sku_as_product_name: false, import_action: doc[:map][:import_action], map: doc[:map][:map], controller: 'stores', action: 'csv_do_import', store_id: @store.id, name: doc[:name], type: 'kit', flag: 'file_upload' }
  ...
  describe 'ShippingEasy Imports' do
    ...
    it 'Import SE Single Order' do
      ...
      get :get_order_details, {order_no: 'SE_QFRANGE3', store_id: se_store.id }
      expect(response.status).to eq(200)
