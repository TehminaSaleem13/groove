...
RSpec.describe EmailScheduling do
  describe '#should_send_email' do
    ...
    context 'when send_email_on_* is true for the corresponding weekday' do
      before do
        allow(self).to receive(:send_email_on_mon).and_return(true)
        allow(self).to receive(:send_email_on_tue).and_return(true)
      end

      it 'returns true for Monday when send_email_on_mon is true' do
        expect(should_send_email(monday_date)).to be true
      end

      it 'returns true for Tuesday when send_email_on_tue is true' do
        expect(should_send_email(tuesday_date)).to be true
      end
    end

    context 'when send_email_on_* is false for the corresponding weekday' do
      before do
        allow(self).to receive(:send_email_on_mon).and_return(false)
        allow(self).to receive(:send_email_on_tue).and_return(false)
      end

      it 'returns false for Monday when send_email_on_mon is false' do
        expect(should_send_email(monday_date)).to be false
      end

      it 'returns false for Tuesday when send_email_on_tue is false' do
        expect(should_send_email(tuesday_date)).to be false
      end
    end
  end
end
