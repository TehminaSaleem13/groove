      ...
      begin
        ...
        if @subscription && !@subscription.tenant.nil?
          ...
          if @access_restriction
            ...
            ApplyAccessRestrictions.new.delay(:run_at => 10.minutes.from_now, :queue => "apply_access_restrictions_#{tenant_name}").apply_access_restrictions(tenant_name, plan_id)
          end
        end
      rescue Exception => e
        puts e.message
