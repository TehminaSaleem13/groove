  ...
  module MagentoRestV2
    class Client < Groovepacker::MagentoRest::Base
      ...
      def orders
        ...
        last_import = begin
                        credential.last_imported_at.to_datetime
        ...
        while page_index
          ...
          filters = {
            ...
            'searchCriteria%5Bfilter_groups%5D%5B0%5D%5Bfilters%5D%5B0%5D%5Bvalue%5D' => (last_import - 10.day).strftime('%Y-%m-%d').to_s,
          ...
          response_length = begin
                              mg_response['items'].length
                            rescue StandardError
                              0
                            end
      ...
      def products(filters = {})
        ...
        while page_index
          ...
          response['items'].each { |product| products[product['id']] = product } unless response['items'].blank?
          response_length = begin
                              response.length
                            rescue StandardError
                              0
