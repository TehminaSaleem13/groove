    ...
    class Products < Groovepacker::Products::Base
      ...
      def create_or_update_product_images
        ...
        (@params[:images] || []).each_with_index do |image, index|
          @result['status'] &= false unless @product.create_or_update_productimage(image, index, product_images)
        end
      end
      ...
      def update_product_basic_info
        ...
        @product.is_intangible = false if type == 'kit'
        @product.restock_lead_time = basic_info[:restock_lead_time].to_i
        @result['status'] &= false unless @product.save
      end

      def attrs_to_update
        %w[disable_conf_req is_kit is_skippable record_serial kit_parsing name product_type
