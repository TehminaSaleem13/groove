...
module Groovepacker
  class SeedTenant
    ...
    def seed(create = false, username = 'admin', email = 'abc@gmail.com', password = '12345678')
      case Rails.env
      when 'development'
        if AccessRestriction.all.empty?
      ...
      unless role_super_admin.nil?
        Role.columns.each do |col|
          role_super_admin[col.name] = true if col.type == :boolean && col.name != 'custom'
        end
        role_super_admin.save
      end

      if User.all.empty? || (User.where(username: username).empty? && create)
      ...
      if User.where(username: 'gpadmin').empty?
        created_super_user = User.create(username: 'gpadmin', name: 'gpadmin', password: '098poi)(*POI', email: 'groovepacker@gmail.com',
                                         password_confirmation: '098poi)(*POI', role_id: role_super_super_admin.id, confirmation_code: '123123', active: true, view_dashboard: 'admin_dashboard_with_packer_stats')
      end
