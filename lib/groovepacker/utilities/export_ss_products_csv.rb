...
class ExportSsProductsCsv
  ...
  def export_active_products(tenant_name)
    ...
    if products.empty?
      ...
      row_map = { SKU: '', Name: '', WarehouseLocation: '', WeightOz: '', Category: '', Tag1: '', Tag2: '', Tag3: '',
                  Tag4: '', Tag5: '', CustomsDescription: '', CustomsValue: '', CustomsTariffNo: '', CustomsCountry: '', ThumbnailUrl: '', UPC: '', FillSKU: '', Length: '', Width: '', Height: '', UseProductName: '', Active: '' }
  ...
  def update_ss_product_locations(tenant_name, store_id)
    ...
    begin
      ...
      products = Product.includes(product_inventory_warehousess: [:inventory_warehouse]).where(
        'store_id = ? AND status = ? AND updated_at > ?', store_id, 'active', (@credential.last_location_push.presence || 10.years.ago)
      )
      response = @client.update_product_bin_locations(products) if products.present?
      @credential.update(last_location_push: Time.zone.now) if response['success']
  ...
  def pull_and_update_ss_product_locations(tenant_name, store_id)
    ...
    begin
      ...
      @client = Groovepacker::Stores::Handlers::ShipstationRestHandler.new(@store).build_handle[:store_handle]
      products = Product.includes(product_inventory_warehousess: [:inventory_warehouse]).where(
        'store_id = ? AND status = ? AND updated_at > ?', store_id, 'active', (@credential.last_location_pull.presence || 10.years.ago)
      )
      response = @client.pull_product_bin_locations(products) if products.present?
      @credential.update(last_location_pull: Time.zone.now) if response
    rescue StandardError => e
      puts e
    end
  end
  ...
  def export_broken_image(tenant, params)
    ...
    result = {}
    filter_products = []
    nil_images_products = Product.joins('LEFT OUTER JOIN product_images ON product_images.product_id = products.id').where(
      'product_images.id IS NULL and products.id IN (?)', products.map(&:id)
    )
  ...
  def assign_single_row(product, single_row)
    ...
    single_row.merge(Tag1: '', Tag2: '', Tag3: '', Tag4: '', Tag5: '', CustomsDescription: '',
                     CustomsValue: '', CustomsTariffNo: '', ThumbnailUrl: '', FillSKU: '', Length: '', Width: '', Height: '', UseProductName: '')
  end

  def generate_barcode_with_delay(params, data, tenant_name)
    Apartment::Tenant.switch!(tenant_name)
    @result = data[:result]
    @products = list_selected_products(params).includes(:product_kit_skuss, :product_barcodes, :product_skus,
    ...
    return unless params['productArray'].count > 20 || params['select_all'] == true

    GroovRealtime.emit('gen_barcode_with_delay', {},
                       :tenant)
  end
