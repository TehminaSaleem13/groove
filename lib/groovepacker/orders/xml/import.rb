    ...
    module Xml
      class Import
        ...
        def process
          ...
          @update_count += 1 if @old_order.try(:attributes) != order.attributes
          ...
          order.order_items.map(&:product).reject(&:blank?).each do |product|
            ...
            next unless intangible_setting_enabled
          ...
          if !@order.import_summary_id.nil? && OrderImportSummary.find_by_id(@order.import_summary_id)
            begin
              ...
              import_item = order_import_summary.import_items if import_item.empty?
              ...
              import_item&.with_lock do
                ...
                @emit_value = (import_item.changes['status'].to_a & %w[not_started completed]).any?
                import_item.save
                if @emit_value
          ...
          order.order_items.includes([product: [:product_skus]]).each do |order_item|
            if @contains_aliased_items
              ...
              unless first_sku.nil?
                ...
                orderXML.order_items.each do |order_item_XML|
                  found = true unless order_item_XML[:product][:skus].index(first_sku).nil?
                end
              end
              order_item.destroy unless found
            end
          end
        end
        ...
        def create_update_product(product, product_xml)
          ...
          if product.save
            ...
            product_xml[:skus].each do |product_sku|
              product.product_skus.create(sku: product_sku) if product.product_skus.where(sku: product_sku).empty?
            end
