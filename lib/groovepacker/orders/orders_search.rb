    ...
    class OrdersSearch < Groovepacker::Orders::Base
      ...
      def get_base_query(search, exact_search, sort_key, sort_order)
        if @params['product_search_toggle'] == 'true'
          ...
          base_query = "Select orders.*, sum(order_items.qty) AS itemslength from orders LEFT JOIN stores ON (orders.store_id = stores.id) LEFT JOIN order_items ON (order_items.order_id = orders.id) WHERE if ((select count(*) from orders where increment_id = #{exact_search})>0,increment_id = #{exact_search},increment_id like #{search} OR tracking_num LIKE '#{'%' + @params[:search]}' OR non_hyphen_increment_id like #{search} OR email like #{search} OR CONCAT(IFNULL(firstname,''),' ',IFNULL(lastname,'')) like #{search} OR postcode like #{search} OR custom_field_one like #{search} OR custom_field_two like #{search} OR notes_internal like #{search} OR tags like #{search} OR stores.name like #{search} OR LOWER(increment_id) = LOWER(#{exact_search})) GROUP BY orders.id Order BY #{sort_key} #{sort_order}"
        end
        base_query
      end
