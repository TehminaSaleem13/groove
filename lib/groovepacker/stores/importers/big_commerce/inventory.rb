        ...
        class Inventory < Groovepacker::Stores::Importers::Importer
          ...
          def pull_inventories
            ...
            (products || []).each do |product|
              ...
              update_product_inv_for_sync_option(product, bc_product, inv_wh) if bc_product['id']
            rescue Exception => e
              return e
            end
          end
          ...
          def pull_single_product_inventory(product)
            ...
            begin
              ...
              bc_product_id = @sync_optn.bc_product_id
              bc_product = @client.product(bc_product_id)
              update_product_inv_for_sync_option(product, bc_product, inv_wh) if eligible_for_sync(bc_product)
            rescue Exception => e
              return e
            end
          end
          ...
          def update_product_inv_for_sync_option(_product, bc_product, inv_wh)
          ...
          def eligible_for_sync(bc_product)
            @sync_optn.sync_with_bc && @sync_optn.bc_product_id && @sync_optn.bc_product_sku && bc_product['id']
          end
        end
      end
    end
