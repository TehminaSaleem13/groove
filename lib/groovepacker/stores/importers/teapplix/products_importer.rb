      ...
      module Teapplix
        class ProductsImporter < Groovepacker::Stores::Importers::Importer
          ...
          def import
            ...
            send_products_import_email(import_products_count) if import_products_count > 20_000
          ...
          def iterate_products_array(response)
            response['products'].each do |teapplix_product|
              ...
              begin
                  create_single_product(teapplix_product)
          ...
          def create_single_product(teapplix_product)
            product = if teapplix_product['sku'].blank?
                        # if sku is nil or empty
                        create_product_with_temp_sku(teapplix_product)
                      elsif @bulk_import || ProductSku.where(sku: teapplix_product['sku']).empty?
                        # if non-nil sku is not found
                        create_new_product(teapplix_product, teapplix_product['sku'])
                      else
                        ProductSku.where(sku: teapplix_product['sku']).first.product
                      end
            product
          end
          ...
          def create_product_with_temp_sku(teapplix_product)
            products = begin
                           Product.where(name: teapplix_product['item_title'])
                       rescue StandardError
                         []
                         end
          ...
          def create_sync_option_for_product(product, _teapplix_product, sku)
