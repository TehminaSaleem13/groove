        ...
        class OrdersImporter < CsvBaseImporter
          ...
          def iterate_and_import_rows(final_records, order_map, result)
            ...
            final_records.each_with_index do |single_row, index|
              ...
              product_name = begin
                               single_row[mapping['product_name'][:position]]
                             rescue StandardError
                               nil
                             end
              product_sku = begin
              ...
              if (index != 0) && current_inc_id.present? && (current_inc_id != inc_id)
                begin
              ...
              next unless final_records.count == index + 1
          ...
          def check_and_destroy_order(single_row)
            inc_id = @helper.get_row_data(single_row, 'increment_id').strip
            begin
              Order.find_by_increment_id(inc_id).destroy
            rescue StandardError
              nil
            end
          end
          ...
          def check_order_with_item(order_items_ar, index, current_inc_id, order_map, result)
            ...
            if origional_order_id
              @order.increment_id = origional_order_id.to_s
              @order.save
            end
          ...
          def import_single_order(single_row, index, inc_id, order_map, result)
            if @helper.not_imported?(@imported_orders, inc_id) || params[:only_for_tracking_num]
              @order = Order.find_or_initialize_by(increment_id: inc_id.to_s)
          ...
          def import_item_failed_result(result, index)
            ...
            @import_item.message = "Import halted because of errors, the last imported row was #{index} Errors: #{result[:messages].join(',')}"
            @import_item.save
          end

          def update_result(result, single_row)
            unless @order_required.empty?
              result[:status] = false
          ...
          def import_order_time(single_row, result)
            params[:order_placed_at] = DateTime.now.in_time_zone.to_s if params[:order_date_time_format] == 'Default'
          ...
          def import_for_nonunique_order_items(single_row, index)
            ...
            loc = begin
                    single_row[mapping['bin_location'][:position]]
                  rescue StandardError
                    nil
                  end
          ...
          def import_order_data(order_map, single_row, index)
            ...
            if single_sku.blank?
              existing_product = begin
                                   Product.find_by_name(single_row[mapping['product_name'][:position]])
                                 rescue StandardError
                                   nil
                                 end

              unless params[:only_for_tracking_num]
                single_row[mapping['sku'][:position]] = if existing_product.present?
                                                          existing_product.primary_sku
                                                        else
                                                          begin
                                                            ProductSku.get_temp_sku
                                                          rescue StandardError
                                                            nil
                                                          end
                                                        end
          ...
          def update_order_increment_sku(product_skus)
            @order_increment_sku = "#{@order_increment_sku}-#{(product_skus.length + 1)}"
          end
          ...
          def save_order_and_update_count(result)
            begin
              ...
              @imported_orders[origional_order_id.to_s] = true if origional_order_id
              @import_item.success_imported += 1
              @import_item.save
            rescue ActiveRecord::RecordInvalid => e
              messages = begin
                           (@order.errors.full_messages << e.message)
                         rescue StandardError
          ...
          def origional_order_id
            @order.increment_id.split('-currupted').first
          rescue StandardError
            nil
          end
          ...
          def remove_already_imported_rows(final_records)
            ...
            if (final_records_size == @csv_import_summary.file_size.to_f) && params[:file_name].strip == @csv_import_summary.file_name
              log_entries = begin
                              @csv_import_summary.csv_import_log_entries.map(&:index).uniq
                            rescue StandardError
                              []
                            end
              ...
              final_records = new_records if (new_records.count + log_entries.count) == final_records.count
              begin
                Order.csv_already_imported_warning if final_records.flatten.uniq.count == 1
              rescue StandardError
                nil
              end
            end
          ...
          def row_already_imported(final_records, single_row, index)
            if (single_row.count == 1) && (single_row.last == 'already_imported')
              true
