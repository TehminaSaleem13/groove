      ...
      module CSV
        class ProductsImporter < CsvBaseImporter
          ...
          def import
            ...
            Product.last(@products_to_import.length).each { |product| product.add_product_activity('Product Import', product.store.try(:name).to_s) }
            found_products_raw = []
            @all_unique_ids.each_slice(20_000) do |ids|
          ...
          def build_single_row_skus(single_row)
            single_row_skus = []
            prim_skus = begin
                          single_row[mapping['sku'][:position]].split(',')
          ...
          def map_barcodes(barcode, order, single_row, usable_record)
            if !mapping[barcode].nil? && mapping[barcode][:position] >= 0
              unless single_row[mapping[barcode][:position]].nil?
                ...
                barcodes_qty = begin
                                 single_row[mapping["#{barcode}_qty"][:position]].split(',')
                               rescue StandardError
                                 1
                               end
          ...
          def prepare_to_import
            found_skus_raw = []
            @all_skus.each_slice(20_000) do |skus|
            ...
            @found_skus = {}
            found_barcodes_raw = []
            @all_barcodes.each_slice(20_000) do |barcodes|
          ...
          def create_single_import(record)
            ...
            single_import.status = !record[:skus].empty? && !record[:barcodes].empty? ? 'active' : 'new'
            single_import
          end
          ...
          def update_existing_prod(duplicate_found, record, _index)
            ...
            duplicate_product.name = record[:name] if !mapping['product_name'].nil? && record[:name] != '' && record[:name] != '[DELETE]'
            ...
            duplicate_product.product_type = record[:product_type] unless mapping['product_type'].nil? # && self.mapping['product_type'][:action] == 'overwrite'
            if !mapping['product_instructions'].nil? && record[:packing_instructions] != '[DELETE]' # && self.mapping['product_instructions'][:action] == 'overwrite'
              duplicate_product.packing_instructions = record[:packing_instructions]
            end
            if !mapping['receiving_instructions'].nil? && record[:product_receiving_instructions] != '[DELETE]' # && self.mapping['receiving_instructions'][:action] == 'overwrite'
            ...
            # if !mapping['inv_wh1'].nil? || !mapping['location_primary'].nil? || !mapping['location_secondary'].nil? || !mapping['location_tertiary'].nil?
              ...
              attributes_location_qty.each do |location_qty|
                default_inventory.send(location_qty + '=', updatable_record[location_qty.to_sym]) if !mapping[location_qty].nil? && updatable_record[location_qty.to_sym] != '[DELETE]'
              end

              attributes_alert_levels = %w[product_inv_alert_level product_inv_target_level]
              attributes_alert_levels.each do |alert_level|
                default_inventory.send(alert_level + '=', updatable_record[alert_level.to_sym]) if !mapping[alert_level].nil? && updatable_record[alert_level.to_sym] != '[DELETE]'
              end
            ...
            if !mapping['category_name'].nil? && !record[:cats].empty?
              ...
              record[:cats].each do |single_to_add_cat|
                next unless !to_not_add_cats.include?(single_to_add_cat) && single_to_add_cat != '[DELETE]'
            ...
            if !mapping['barcode'].nil? && !record[:barcodes].empty?
              ...
              record[:barcodes].each_with_index do |single_to_add_barcode, index|
                next unless !to_not_add_barcodes.include?(single_to_add_barcode) && single_to_add_barcode != '[DELETE]'

                begin
                  to_add_barcode = ProductBarcode.new
                  to_add_barcode.barcode = single_to_add_barcode
                  to_add_barcode.packing_count = begin
                                                     record[:all_barcodes_qty][single_to_add_barcode][0].to_i
                                                 rescue StandardError
                                                   1
                                                   end
            ...
            if !mapping['sku'].nil? && !record[:skus].empty?
              ...
              all_found_skus.each do |single_found_dup_sku|
                to_not_add_skus << single_found_dup_sku.sku if record[:skus].include? single_found_dup_sku.sku
              end
              record[:skus].each_with_index do |single_to_add_sku, index|
                next unless !to_not_add_skus.include?(single_to_add_sku) && single_to_add_sku != '[DELETE]'
            ...
            if !mapping['product_images'].nil? && !record[:images].empty?
          ...
          def build_usable_records
            final_record.each_with_index do |single_row, index|
              ...
              if (!mapping['sku'].nil? && mapping['sku'][:position] >= 0 && !single_row[mapping['sku'][:position]].blank?) || (!mapping['remove_sku'].nil? && mapping['remove_sku'][:position] >= 0 && !single_row[mapping['remove_sku'][:position]].blank?)
                single_row_skus = build_single_row_skus(single_row)
                !(@all_skus & single_row_skus).empty? ? @duplicate_file += 1 : insert_usable_record_skus(index, single_row_skus, single_row)
              end
          ...
          def import_or_overwrite_prod
            @usable_records.each_with_index do |record, index|
              ...
              @success += 1
              next unless (index + 1) % @check_length === 0 || index === (@usable_records.length - 1)
          ...
          def set_single_row_skus(single_row, single_row_skus, sku_type)
            if !mapping[sku_type.to_s].nil? && mapping[sku_type.to_s][:position] >= 0
              unless single_row[mapping[sku_type.to_s][:position]].nil?
                other_skus = single_row[mapping[sku_type.to_s][:position]].split(',')
          ...
          def update_new_product_attributes(index)
            return @product_import.save unless (index + 1) % @check_length === 0 || index === (final_record.length - 1)
          ...
          def insert_usable_record_skus(index, single_row_skus, single_row)
            ...
            begin
              ...
              sku_types.each do |single_sku_type|
                usable_record[:new_sku] << single_row[mapping[single_sku_type][:position]].split(',')[0]
              rescue StandardError
                nil
              end
          ...
          def assign_duplicate_product_attributes(record, duplicate_product)
            ...
            attributes1.each do |attribute_name|
              duplicate_product.send(attribute_name + '=', record[attribute_name.to_sym]) if !mapping[attribute_name].nil? && record[attribute_name.to_sym] != '[DELETE]'
            end
            ...
            attributes2.each do |attribute_name|
              duplicate_product.send(attribute_name + '=', record[attribute_name.to_sym]) if !mapping[attribute_name].nil? && record[attribute_name.to_sym] != '' && record[attribute_name.to_sym] != '[DELETE]'
            end
            ...
            if !mapping['count_group'].nil? && record[:count_group] != '[DELETE]' && record[:count_group].present?
              duplicate_product.count_group = record[:count_group].chars.first
            end

            if !mapping['restock_lead_time'].nil? && record[:restock_lead_time] != '[DELETE]' && record[:restock_lead_time].present?
              duplicate_product.restock_lead_time = record[:restock_lead_time].to_i
            end

            if !mapping['avg_cost'].nil? && record[:avg_cost] != '[DELETE]' && record[:avg_cost].present?
          ...
          def delete_product(record)
            ...
            record[:new_sku].each_with_index do |sku, new_order|
              product.product_skus.find_by_order(new_order).destroy if sku == '[DELETE]' && product.product_skus.count > 1
            rescue StandardError
              nil
            end
