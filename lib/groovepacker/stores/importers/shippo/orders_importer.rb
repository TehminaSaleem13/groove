      ...
      module Shippo
        class OrdersImporter < Groovepacker::Stores::Importers::Importer
          ...
          def import
            ...
            @credential.update_attributes(last_imported_at: Time.zone.parse(response['orders'].last['to_address']['object_updated'])) rescue nil if @import_item.status != 'cancelled'
          ...
          private
            ...
            def import_single_order(order)
              ...
              if order_in_gp
                ...
                update_import_count('success_updated') && return if is_scanned || (order_in_gp.last_modified == Time.zone.parse(order['to_address']['object_updated']))
            ...
            def add_order_activities(shippo_order)
              shippo_order.addactivity("Order Import", @store.name+" Import")
