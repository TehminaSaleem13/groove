groovepacks_controllers.controller('storeSingleModal', ['$http', '$scope', 'store_data', '$window', '$sce', '$interval', '$state', '$stateParams', '$modal',
  ...
  function ($http, scope, store_data, $window, $sce, $interval, $state, $stateParams, $modal, $modalInstance, $timeout, hotkeys, stores, warehouses, notification, $q, groov_translator, Lightbox) {
    ...
    scope.popup_shipping_label = function(){
      store_id = scope.stores.single.id
      $http.get('/stores/popup_shipping_label.json?store_id=' + store_id).success(function (data) {
    ...
    scope.use_chrome_extention = function(){
      store_id = scope.stores.single.id
      $http.get('/shipstation_rest_credentials/use_chrome_extention.json?store_id=' + store_id).success(function (data) {
