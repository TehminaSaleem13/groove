                ...
                '<td style="text-align:right;width:38%;padding:3px;">' +
                  '<div class="btn-group">' +
                    '<div ng-show="store.store_type==\'BigCommerce\'" style="display: flex;">' +
                      ...
                      '<div ng-mouseover="show_days_select(store, true)" ng-mouseleave="show_days_select(store, false)" style="width: 120px;">' +
                        '<a class="btn" ng-hide="import_summary.import_info.status==\'in_progress\'" title="Deep Import" ng-click="issue_import(store.id, store.days, \'deep\')" style="float: left;"><img class="icons" src="/assets/images/deep_import.png"></img></a>' +
                        '<input type="number" ng-model="store.days" data-import="{{store.id}}" ng-mouseleave="check_days_value(store)" style="display: none;font-size: 15px;height: 30px;width: 50px;"/>' +
                      '</div>' +
                    '</div>' +
