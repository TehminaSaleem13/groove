...
controller('showStoresCtrl', [ '$scope', '$http', '$timeout', '$stateParams', '$location', '$state', '$cookies', 'warehouses',
function( $scope, $http, $timeout, $stateParams, $location, $state, $cookies, warehouses) {
        ...
        $http.get('/store_settings/getstoreinfo.json?id='+id).success(function(data){
            ...
            if (data.status)
            {
                ...
                if (data.store.store_type == 'Amazon')
                {
                    ...
                    $scope.newStore.show_shipping_weight_only = data.credentials.amazon_credentials.show_shipping_weight_only;
