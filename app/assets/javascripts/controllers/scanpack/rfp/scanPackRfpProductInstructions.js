groovepacks_controllers.controller('scanPackRfpProductInstructions', [ '$scope', '$modalInstance', '$timeout', 'order_data', 'scanPack',
function($scope,$modalInstance,$timeout,order_data,scanPack){
    ...
    $scope.update = function(reason) {
        ...
        if(reason != "cancel-button-click") {
            scanPack.product_instruction($scope.order.id,$scope.order.next_item, $scope.confirmation_code).then(function(data) {
