        ...
        function( $scope, $http, $timeout, $stateParams, $location, $state, $cookies, products, scanPack) {
            ...
            $scope.editreload = function () {
               return $scope.rfpinit().then(function() {
                    $scope.set('title',"Fix status items or scan a new order");
            ...
            $scope.editinit = function() {
                ...
                $scope.gridOptions = {
                    ...
                    all_fields: {
                        name: {
                            name: "Item Name",
                            grid_bind: '<a ui-sref="scanpack.rfp.product_edit.product({order_num: options.data.order_num, id: row.id })" >{{row[field]}}</a>'
                        },
                        status: {
                            name: "Status"
                        }
            ...
            if($state.current.name == 'scanpack.rfp.product_edit') {
                $scope.editinit();
            }
        }]);
