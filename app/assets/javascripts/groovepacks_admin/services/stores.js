groovepacks_admin_services.factory('stores', ['$http', 'notification', '$filter', function ($http, notification, $filter) {
  ...
  var ebay_token_delete = function (stores) {
    return $http.get('/store_settings/delete_ebay_token.json?storeid=' + stores.single.id).success(function (data) {
      if (data.status) {
        ebay_sign_in_url(stores);
      }
    }).error(notification.server_error);
  ...
  var ebay_token_update = function (stores, id) {
    return $http.get('/store_settings/update_ebay_user_token.json?storeid=' + id).success(function (data) {
      if (data.status) {
        stores.ebay.show_url = false;
      }
    }).error(notification.server_error);
  ...
  var csv_do_import = function (csv) {
    return $http.post('/store_settings/csv_do_import.json', csv.current).success(function (data) {
      if (data.status) {
  ...
  var csv_product_import_cancel = function (id) {
    return $http.post('/store_settings/csv_product_import_cancel.json', {id: id}).success(function (data) {
  ...
  var update_csv_map = function (stores, map) {
    return $http.post('/store_settings/update_csv_map.json', {
  ...
  var delete_csv_map = function (stores, kind) {
    return $http.post('/store_settings/delete_csv_map.json', {
      store_id: stores.single.id,
      kind: kind
    }).success(function (data) {
      if (data.status) {
