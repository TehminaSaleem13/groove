groovepacks_services.factory('stores', ['$http', 'notification', '$filter', function ($http, notification, $filter) {
  ...
  var delete_map = function(stores, map){
    $http.post('/stores/'+stores.single.id+'/delete_map.json', {map: map})
  };
  ...
  var update_product_image = function(store_id){
    return $http.put('/shipstation_rest_credentials/' + store_id + '/update_product_image.json', null).success(function(data) {
      if (data.status) {
        notification.notify(data.messages, 1);
      } else {
        notification.notify(data.messages, 0);
