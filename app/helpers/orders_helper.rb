...
module OrdersHelper
  ...
  def filter_orders
    ...
    column_mapping = {
      ...
      "Items" => "itemslength",
      "Recipient" => "CONCAT(firstname, '', lastname)",
      "Status" => "status",
    ...
    filters.each do |filter|
      ...
      case operator
      when 'contains'
        if attribute_name == 'Store'
          orders = all_orders.joins(:store).where("LOWER(stores.name) LIKE ?", "%#{value.downcase}%")
        else
          orders = all_orders.where("LOWER(#{db_column_name}) LIKE ?", "%#{value.downcase}%")
        end
      when 'noContains'
        if attribute_name == 'Store'
          orders = all_orders.joins(:store).where.not("LOWER(stores.name) LIKE ?", "%#{value.downcase}%")
        else
          orders = all_orders.where.not("LOWER(#{db_column_name}) LIKE ?", "%#{value.downcase}%")
        end
      when 'eq', 'neq', 'before', 'after', 'beforeOrOn', 'afterOrOn', 'gte', 'gt', 'lte', 'lt'
        if type == 'date'
          ...
          orders = all_orders.joins(:order_items)
            .select("orders.*, SUM(CAST(order_items.qty AS SIGNED)) AS total_qty")
            .group("orders.id")
            .having("CAST(total_qty AS SIGNED) #{operation_map[operator]} ?", value.to_i)
        elsif attribute_name == 'Store'
          orders = all_orders.joins(:store).where("stores.name #{operation_map[operator]} ?", value.downcase)
        elsif type == 'date'
          value = DateTime.strptime(value, "%m-%d-%Y") if value.is_a?(String)
          orders = all_orders.where("DATE(#{db_column_name}) #{operation_map[operator]} ?", value)
        else
      ...
      when 'inrange', 'notinrange'
        orders = process_range_filter(all_orders, db_column_name, operator, value, type)
      else
        orders = []
      end
      final_order = orders
  ...
  def process_range_filter(orders, db_column_name, operator, value, type)
    if type == 'number' && value["start"].present? && value["end"].present?
      orders = orders.joins(:order_items)
        .where("CAST(order_items.qty AS SIGNED) >= CAST(? AS SIGNED) AND CAST(order_items.qty AS SIGNED) <= CAST(? AS SIGNED)", value["start"].to_i, value["end"].to_i).distinct
      # orders = Order.where({ db_column_name => value["start"].to_i..value["end"].to_i })
    elsif type == 'date' && value["start"].present? && value["end"].present?
      start_date = DateTime.strptime(value["start"], "%m-%d-%Y")
      end_date = DateTime.strptime(value["end"], "%m-%d-%Y")
      orders = Order.where({ db_column_name => start_date..end_date })
    end
    orders
  end
