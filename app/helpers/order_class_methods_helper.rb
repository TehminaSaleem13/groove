...
module OrderClassMethodsHelper
  def create_new_order(result, _current_user)
  ...
  def emit_notification_ondemand_quickfix(user_id)
    result = { 'status' => true, 'message' => 'It appears that the order scanned is more recent than any of the orders that have been imported. An import will be run to fetch all new orders. Please continue scanning once the import completes.', 'user_id' => user_id.to_s }
  ...
  def emit_data_for_on_demand_import_v2(hash, order_no, user_id)
    if hash['orders'].blank?
      ...
      result = { 'status' => true, 'message' => "Order #{order_no} is Ready and will open in:", 'id' => order_no.to_s, 'user_id' => user_id.to_s }
      GroovRealtime.emit('popup_display_for_on_demand_import_v2', result, :tenant)
    end
  end

  def emit_notification_all_status_disabled(user_id)
    result = { 'status' => true, 'message' => "All import option switches are disabled. Please refer to <a target='_blank' href='https://groovepacker.freshdesk.com/en/support/solutions/articles/6000058013-shipstation-order-status-tag-import-options'><u style='color: antiquewhite;'>this article</u></a> for help.", 'user_id' => user_id.to_s }
  ...
  def multiple_orders_scanning_count(orders)
    ...
    grouped_data.each do |order_id, order_data|
      orders_scanning_count[order_id] =
        order_data.each_with_object(scanned: 0, unscanned: 0) do |data_hash, tmp_hash|
  ...
  def update_grouped_order_date(tmp_hash, data_hash)
    if data_hash['is_kit'] == 1
      ...
      unless data_hash['is_intangible'] == 1
        tmp_hash[:unscanned] += (begin
                                   data_hash['order_item_qty'] - data_hash['order_item_scanned_qty']
  ...
  def get_temp_increment_id
    ...
    while true
      next_inc_id = if temp_increment_ids.length.positive?
                      'GP-Manual-Order-' + (get_last_temp_increment_id(temp_increment_ids) + 1).to_s
                    else
                      'GP-Manual-Order-1'
                    end
