...
module ScanPack
  class ProductScanService < ScanPack::Base
    ...
    def initialize(args)
      @current_user, @session, @input, @state, @id, @box_id, @on_ex, @typein_count, @type_scan, @extras = args
      @extras ||= {}
      @result = {
    ...
    def product_scan(clicked, serial_added)
      ...
      if @scanpack_settings.restart_code_enabled? && @input == @scanpack_settings.restart_code
        ...
        do_if_remove_or_partial_code_is_enabled_and_and_eql_to_input('REMOVE-ALL')
    ...
    def do_if_remove_or_partial_code_is_enabled_and_and_eql_to_input(code_type)
      ...
      if code_type == 'REMOVE-ALL'
        ...
        @single_order.addactivity("QTY #{qty} of SKU #{item['product_type'] == 'individual' ? child_item['sku'] : item['sku']} was removed using the REMOVE barcode", @current_user.try(:username), @on_ex)
      end
      do_if_barcode_found
    end
      ...
      if @single_order.has_unscanned_items
        ...
        when 'items_sequence'
          ...
          if check_scanning_item(unscanned_items, clean_input)
            ...
            @single_order.addactivity("OUT OF SEQUENCE - Product with barcode: #{unscanned_items.first['child_items'].first['barcodes'].map { |barcode| barcode['barcode'] }.first} was suggested and barcode: #{clean_input} was scanned", @current_user&.username || 'gpadmin')
            @result['status'] &= false
