			    ...
			    File.open("public/#{file_name}", 'a+', {force_quotes: true}) do |csv|	
			    	products.each_with_index do |pro, index|			    		
				    	...
				    	inv = pro.product_inventory_warehousess
							orders = Order.where("id IN (?) and scanned_on >= ? and scanned_on <= ?",orders.map(&:id), product_inv_setting.start_time.beginning_of_day, product_inv_setting.end_time.end_of_day)
