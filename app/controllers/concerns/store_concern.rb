...
module StoreConcern
  ...
  def order_csv_mapping(csv_map, csv_directory, current_tenant, default_csv_map)
    ...
    if %w[both order].include?(params[:type])
      ...
      if csv_map.order_csv_map.blank?
        ...
        begin
          new_map = temp_mapping[:map].each_with_object({}) { |(k, v), hash| hash[k] = (v['value'].in?(%w[custom_field_one custom_field_two]) ? v.merge('name' => general_settings[v['value']]) : v); }
        rescue Exception => e
          new_map = temp_mapping[:map].to_unsafe_h.each_with_object({}) { |(k, v), hash| hash[k] = (v['value'].in?(%w[custom_field_one custom_field_two]) ? v.merge('name' => general_settings[v['value']]) : v); }
        end
  ...
  def product_kit_csv_map(csv_map, csv_directory, current_tenant, default_csv_map)
    ...
    if %w[both kit].include?(params[:type])
      @result['kit'] = {}
      @result['kit']['map_options'] = [{ value: 'kit_sku', name: 'KIT-SKU' }, { value: 'kit_name', name: 'KIT-NAME' }, { value: 'kit_barcode', name: 'KIT-BARCODE' }, { value: 'part_sku', name: 'PART-SKU' }, { value: 'part_name', name: 'PART-NAME' }, { value: 'part_barcode', name: 'PART-BARCODE' }, { value: 'part_qty', name: 'PART-QTY' }, { value: 'scan_option', name: 'SCAN-OPTION' }, { value: 'kit_part_scanning_sequence', name: 'KIT-PART-SCANNING-SEQUENCE' }]
  ...
  def check_csv_condition
    (params[:type] == 'order' && current_user.can?('import_orders')) || (params[:type] == 'both' && current_user.can?('import_orders') && current_user.can?('import_products')) || (%w[product kit].include?(params[:type]) && current_user.can?('import_products'))
  end
  ...
  def check_store_status
    ...
    if (params[:type] == 'order' && !current_user.can?('import_orders')) || (%w[product kit].include?(params[:type]) && !current_user.can?('import_products'))
      @result['status'] = false
  ...
  def push_pull_inventory(flag)
    ...
    import_orders_obj.delay(run_at: 1.seconds.from_now, queue: "start_import_#{Apartment::Tenant.current}", priority: 95).init_import(tenant)
    if @store && current_user.can?('update_inventories')
      ...
      if @store.store_type == 'Shopify'
        ...
        if flag == 'push'
          context.delay(run_at: 1.seconds.from_now, queue: 'push_inventory', priority: 95).push_inventory
        elsif flag == 'pull'
          context.delay(run_at: 1.seconds.from_now, queue: 'pull_inventory', priority: 95).pull_inventory
