...
class BigCommerceController < ApplicationController
  before_action :groovepacker_authorize!, only: %i[check_connection disconnect]
  before_action :find_store, only: %i[check_connection disconnect]

  def setup
    # redirect to admin page with the big-commerce and with groove-solo plan
    # get shop name
    @store_user_email = begin
                          cookies[:store_user_email]
  ...
  def bigcommerce
    ...
    app_session = begin
                    JSON.parse(app_session)
                  rescue StandardError
                    {}
                  end
    if app_session['tenant'].blank?
  ...
  def update_bc_credentials
    ...
    @bigcommerce_credentials.access_token = begin
                                                @auth_hash['access_token']
                                            rescue StandardError
                                              nil
                                              end
    @bigcommerce_credentials.store_hash = begin
                                              @auth_hash['context']
                                          rescue StandardError
                                            nil
                                            end
  ...
  def store_auth_values_in_cookies
    store_user_email = begin
                           @auth_hash['user']['email']
                       rescue StandardError
                         ''
                         end
    store_access_token = begin
                             @auth_hash['access_token']
                         rescue StandardError
                           ''
                           end
