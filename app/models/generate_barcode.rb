...
class GenerateBarcode < ApplicationRecord
  ...
  def emit_data_to_user
    GroovRealtime.user_emit('pnotif', { type: 'generate_barcode_status', data: self, post_scanning_option: ScanPackSetting.last.post_scanning_option }, user_id)
  end

  def self.generate_barcode_for(orders, current_user, print_type = nil)
    ...
    g_barcode.print_type = print_type
    g_barcode.save
    g_barcode
  end
end
