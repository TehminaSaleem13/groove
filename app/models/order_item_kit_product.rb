class OrderItemKitProduct < ActiveRecord::Base
  ...
  belongs_to :product_kit_skus, optional: true
  ...
  def set_clicked_qty(clicked, username, typein_count)
    if clicked
      ...
      self.order_item.order.addactivity("Kit Part item with SKU: " + self.cached_product_kit_skus.cached_option_product.cached_primary_sku + " has been click scanned", username) if !ScanPackSetting.last.order_verification
    end
  end
